...
The initial alpha scaffold is in place. Live state-change capture, the bounded
event store, UI configuration, translations, and the first query-action shapes
exist, but they still require Home Assistant integration tests. Recorder
restoration is the next critical task. This snapshot is not production-ready.
...
- Climate manual-override example validated.
